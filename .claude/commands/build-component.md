...
**Text content check:** For every child component instance that has variable text (e.g. Pill labels, button text, badge counts), record the exact text shown in the Figma design context output and use that — do not invent placeholder values. Text content has the same fidelity requirement as tokens. Concrete mistake: VersionHistory Pill label was set to "2 minutes ago" (invented) instead of "Live" (what Figma shows).
...
- **Modal scroll regions use `.modal__scroll`.** Any element that scrolls inside a dialog gets
  ...
  the designer's request: "apply it to all overflow modal scrolls moving forward." Figma specifies
  no scrollbar at all — its Modal `Type=Scrollable` variant draws a clipped container — so this is
  a code convention, which is exactly why it has to be one value and not per-component taste.

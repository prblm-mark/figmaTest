...
## Project Overview
The Affino Design System — design system for Affino's client-facing Control Centre, AI
...
- **Key lessons:** Always check for explicit `h-[...]` height tokens (not just padding). Tertiary = white/no border (not same as secondary). Button typography = `--ai-font-fluid-xs` + `--ai-leading-xs`, NOT fixed-sm + leading-md. (Leading tokens renamed Mar 2026: 1→xs, new sm=1.25rem, 2→md, 3→lg, 4→xl, 5→2xl) Form filled text = `--ai-text-primary`, placeholder = `--ai-text-contrast` (different!). Use `:has()` for clear button visibility.

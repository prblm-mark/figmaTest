...
| `seating-room-list` | Room Card (`src/components/RoomCard/`) — DS component from Figma `3470:84951` | Room name, tables, seated and capacity are hardcoded; the capacity bar is driven by a `--room-card-progress` custom property the caller sets, so the component computes nothing. Selection is a class the parent toggles — no JS ships, so nothing coordinates single-select. Edit and delete are unwired | `GET /api/events/{eventId}/rooms` → `[{ id, name, tableCount, seatsSeated, seatsCapacity }]`. **Derive the counts server-side** — the same trap as `seating-plan-chip-summary`, where a written-once summary went stale after an unseat and silently lied. Delete needs a confirmation and a decision about the tables and seated attendees underneath. **Resolve first:** this card's content is identical to the plan chip — confirm whether a "Room" is a new entity or whether Room Card is the componentised SeatingPlan chip, in which case the prototype's chip should be replaced by it | needs-backend |

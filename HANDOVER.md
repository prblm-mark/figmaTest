...
| `seating-main-layout` | Main interface, TASK-344753 — `src/prototypes/SeatingPlanner/workspace.html` | **DECIDED 31 Jul 2026: the worklist layout is the main interface.** The other two alternatives, the compare board, the layout switcher and its demo bar, and the four old fallback workspace states are **deleted** — `workspace.html` is the only workspace file. Tables list \| detail \| unassigned, each region owning its own scroller so the **page never scrolls** and both ends of a drag stay on screen (the defect it replaced put the pool's first row at y≈1013 in a page-scrolled 1356px document). Detail `minmax(320, 384)` and pool 320, both **draggable wider** and never narrower; tables list flips between rows and 240px cards, grid by default; below 1024 the pool is hidden, the toggle goes and the detail opens **inline beneath the tapped row**. **All 13 sub-tasks are wired here** — verified by clicking every `[data-sp-action]` at 1440 and 500 and asserting the shell's *"not built in this task"* toast never appears, plus plan switch, table select, reorder, unseat, drag, filters, both searches, export split menu. Two things had **never** been wired in any layout and were fixed to get here: **Table types** (342308) had a handler but no trigger anywhere, and **Change event** (342608) had no handler at all | Build from `workspace.html`, not from the old frames. The shell CSS/JS stays `.sp-shell`-scoped and additive because `SeatingPlanner.css` also paints screen 1 and the 34 sub-task capture states — those still carry the old `.sp-ws__cols` markup, which is why `DeletePlan.js` keeps that selector beside the `[data-sp-workspace]` hook. Still open: `seating-card-keyboard`, `seating-workspace-prefs`, `seating-card-left-border` | fe-wired |
| `seating-room-list` | Room Card (`src/components/RoomCard/`) — DS component from Figma `3470:84951` | Room name, tables, seated and capacity are hardcoded; the capacity bar is driven by a `--room-card-progress` custom property the caller sets, so the component computes nothing. Selection is a class the parent toggles — no JS ships, so nothing coordinates single-select. Edit and delete are unwired | **RESOLVED: a Room Card IS a room/plan card** — the componentised plan chip, not a new entity above `SeatingPlan`, so **no new table is needed**. Name is the plan name; tables/seated/capacity derive from the plan's `Table` + `TableSeat` rows. `GET /crm/events/{eventId}/plans` → `[{ id, name, tableCount, seatsSeated, seatsCapacity }]`. **Derive the counts server-side** — the exact trap in `seating-plan-chip-summary`, where a written-once summary went stale after an unseat and silently lied. Delete needs a confirmation and a decision about the tables and attendees underneath (cf. `seating-delete-plan`). **FE follow-up:** replace the prototype's hand-built chip with this component | needs-backend |
| `seating-table-grid` | Table Card (`src/components/TableCard/`) — DS component from Figma `3470:85483` | Table name, tier, sponsor, per-role seat counts and capacity all hardcoded. The bar is proportional but the component computes nothing — each segment's count arrives as a `--seg` custom property the caller sets. Selection is a class the parent toggles; edit and delete are unwired | `GET /crm/plans/{planId}/tables` → `[{ id, name, tableType, sponsor, capacity, seats: [{ role }] }]`. **Aggregate the per-role counts server-side** and pass them to `--seg` — same trap as `seating-plan-chip-summary`. The role set must match `--sp-*`: **Figma's legend omits Host** though the collection and AttendeeCard both define it, so settle whether a table can seat one. Tier from `seating-table-types`, sponsor from `seating-table-form`, edit/delete route to `seating-edit-table` / `seating-delete-table`. The bar is `aria-hidden`, so the legend text is the accessible representation and must carry real counts | needs-backend |
| `seating-table-detail` | Table Detail (`src/patterns/TableDetail/`) — DS pattern from Figma `3475:94010` | Table name, tier, sponsor, seated count and every seat row hardcoded; the role legend is hand-written in the demo. Panel fills its column and the seat list scrolls, but delete, reorder, Assign and the drop target are all unwired | `GET /crm/tables/{tableId}` → `{ id, name, tableType, sponsor, capacity, seats: [{ index, attendee, role }] }`. **The legend is DERIVED from the roles actually seated — do not store it** (that is why Type=Default has none rather than an empty one). Derive the seated count server-side, same trap as `seating-plan-chip-summary`. Assign → `seating-assign-seat`, delete/reorder → `seating-seat-occupant-record`, drop target → `seating-drag-assign`. Roles must match `--sp-*`; this legend covers all five including Host. **Mobile:** the header is hidden below 768px assuming the sheet supplies the table name — revisit if that chrome changes, or a screen-reader user gets an unnamed seat list | needs-backend |
| `seating-tables-listing` | Table Listing (`src/patterns/TableListing/`) — DS pattern from Figma `3488:196339`: the tables grid plus its toolbar | Table list, counts, search and the "Only free seats" toggle all hardcoded. The toggle is a real `role="switch"` and the search a real `type="search"`, but neither is wired. The grid is responsive on its own | Per-table data reuses **`seating-tables-grid`**; this covers the toolbar. **"Only free seats"** should derive server-side from the same aggregated counts (see `seating-free-seats-filter`), not filter a client list. **Search** by table name should debounce a `?q=` against the tables endpoint so a 200-table plan behaves. **Open:** a hidden `View Actions` frame (two buttons, likely grid/list view) was not built — confirm whether it is coming, since it may need a persisted view preference (cf. `seating-workspace-prefs`) | needs-backend |
| `seating-header` | Seating Header (`src/patterns/SeatingHeader/`) — DS pattern from Figma `3474:90519`: the event info bar, room selector and tables toolbar | Event name, date, attendee count, venue, plan list, active room and its unassigned count are all hardcoded. "Show unassigned" flips itself (Toggle.js) and emits `toggle:change`, but nothing listens. Copy Plans, New Plan, event-switch, Room Layout, Add Table, Export and the kebab are unwired. Has Plans vs No Plans is markup the caller picks | **Event header:** `GET /crm/events/{eventId}` → `{ name, date, attendeeCount, venue }`; the event-switch button implies a picker over `seating-events-list`. **Plan list** reuses **`seating-room-list`** — this row adds only the strip around it. **The unassigned count** is the same aggregate `seating-tables-listing` needs — **derive it server-side, never store it**, the exact trap `seating-plan-chip-summary` documents. **"Show unassigned"** is a view preference (cf. `seating-workspace-prefs`), *not* the same thing as `seating-free-seats-filter`. Actions reuse `seating-copy-plans`, `seating-new-plan`, `seating-add-table`, `seating-export`, `seating-room-layout`. **No Plans should follow from an empty plan list.** **Open:** the kebab menu's contents are undefined — Figma draws only the trigger | needs-backend |
| `seating-toast` | Seating Toast (`src/components/SeatingToast/`) — DS component from Figma `3474:90240`: the action confirmation / warning pill with an Undo | Messages, emphasised names and the Undo button all hardcoded. Presentational — no show/hide, no auto-dismiss, no JS; unlike the `Toast` card there is no close button to wire. `role="status"` / `role="alert"` are in place | **Undo is the part that needs the backend.** Each seating action (`seating-assign-seat`, `seating-drag-assign`, `seating-touch-placement`) must return an **operation id** the toast carries and posts back to reverse *that* operation — a blanket "undo last change" is wrong, since two toasts can be on screen and the user may undo the older one. **Failure text** ("Table 12 is full") should come from the server's rejection, not be composed client-side, so it stays truthful when the rule changes (cf. `seating-capacity-reduction`). **Who retires a toast is an undecided FE question** — the component has no timer by design, and Undo must not vanish before it can be clicked (WCAG 2.2.1). **Open:** `Show Cta = false` is a real Figma boolean neither drawn variant exercises, so the CTA-less 46px height is emergent | needs-backend |
| `seating-prototype-inventory` | What is left in `src/prototypes/SeatingPlanner/` after the cleanup | **40 HTML files**: `workspace.html` (adopted), 4 screen-1 picker states, 34 sub-task capture states, `states.html` (index, not a screen) and `_base-workspace.html` — a **build input**, not a screen: the generator reads it to rebuild `workspace.html` with all ten modal roots intact, and nothing links to it. **Deleted:** `alt1-threepane`, `alt1b-worklist-v2`, `alt3-accordion`, `alt-compare` and `workspace-{no-event,no-plan,plan-selected,free-seats}` | Nothing to do. Recorded because `src/prototypes/` is **untracked** — those eight files have no git history and cannot be recovered, so this row is the only record of what existed and what replaced it. Their Figma frames still exist on page `2025:803`; the superseded ones want deleting by hand | visual-only |

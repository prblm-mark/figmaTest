...
| `seating-deferred-controls` | All remaining `[data-sp-action]` controls | Render + announce their owning task. **Built so far: everything except room-layout 344760 and export 344761** | One sub-task each: room-layout 344760 · export 344761. Note the registry now has **two** kinds of extension point: click actions, and *notify* for a sub-task that owns a derived view (351550) | needs-backend |

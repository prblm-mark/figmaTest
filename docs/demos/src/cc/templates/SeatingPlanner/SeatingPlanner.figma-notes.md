...
### The resting overhang is the decision, not a defect (2026-09-11)

**At rest the three sheets run ~253px below the fold**, because they stand at their stuck height
from the start. That is what gives the page something to scroll, and it is Figma's own model — the
frame only ever draws the scrolled state. The first wheel notch anywhere lines everything up.

This was raised, and the alternative was built and measured before being dropped, so it is worth
recording rather than re-deriving:

| | Natural scroll (**built**) | Condensing header (built, then reverted) |
|---|---|---|
| Page scroll | ~253px, the retiring header | none above 1023 |
| Sheets at rest | overhang the fold by ~253 | fit exactly, never clipped |
| Header goes when | the page is scrolled | any sheet's own content is scrolled |
| Mechanism | sticky, negative inset | `margin-block-start`, transitioned |

The condensing version never clipped, but it traded that for a worse failure: a sheet only condenses
if it has more to scroll than condensing gives back, or growing the sheets wipes out the overflow
that triggered it and the header flip-flops. On a 1728×1139 window with 12 tables the grid has 199px
of travel against a 228px header, so it fell under that guard and **nothing moved at all** — no page
scroll, no condense. Two mechanisms, two thresholds, and a screen that behaved differently on a
large monitor than on a laptop.

The sheets are meant to expand over the viewport (designer, 2026-09-11). Reverted to the natural
